...
namespace ExamWeb.Services
{
    public class ExcelService : IExcelService
    {
        ...
        public Workbook AlumniExportToExcel()
        {
            ...
            int[] maxColumnWidths = new int[13];
            ...
            workSheet.Cells[0, 8].PutValue("Faculty - Major");
            ...
            maxColumnWidths[8] = "Faculty - Major".Length;
            ...
            for (int i = 0; i < data.Count(); i++)
            {
                ...
                workSheet.Cells[i + 1, 7].PutValue(data.ToList()[i].FullAddresses);
                workSheet.Cells[i + 1, 8].PutValue(data.ToList()[i].FullFacultyMajorName);
                workSheet.Cells[i + 1, 9].PutValue(data.ToList()[i].DateOfBirth);
                if ((data.ToList()[i]).DateOfBirth.HasValue)
                {
                    workSheet.Cells[i + 1, 9].PutValue((data.ToList()[i]).DateOfBirth.Value);
                    workSheet.Cells[i + 1, 9].SetStyle(GetDateStyle(workBook));
                }
                ...
                maxColumnWidths[4] = Math.Max(maxColumnWidths[4], data.ToList()[i].Email.Length);
                maxColumnWidths[5] = Math.Max(maxColumnWidths[5], data.ToList()[i].MobileNumber.Length);
                maxColumnWidths[6] = Math.Max(maxColumnWidths[6], data.ToList()[i].Address.Length);
                maxColumnWidths[7] = Math.Max(maxColumnWidths[7], data.ToList()[i].FullAddresses.Length);
                maxColumnWidths[8] = Math.Max(maxColumnWidths[8], data.ToList()[i].FullFacultyMajorName.Length);
                ...
                maxColumnWidths[11] = Math.Max(maxColumnWidths[11], (data.ToList()[i].Degree ?? "").Length);
                maxColumnWidths[12] = Math.Max(maxColumnWidths[12], (data.ToList()[i].LinkedInProfile ?? "").Length);
            }
            ...
            int linkedInProfileColumnIndex = 12;
        ...
        public void ImportAlumniExcelToDatabase(HttpPostedFileBase file)
        {
            ...
            for (int i = 1; i <= worksheet.Cells.MaxDataRow; i++)
            {
                ...
                string FullAddresses = worksheet.Cells[i, 7].StringValue;
                string FullFacultyMajorName = worksheet.Cells[i, 8].StringValue;
                DateTime DateOfBirth = worksheet.Cells[i, 9].DateTimeValue;
